...
public class SearchBarMaterial3ControlPage : NavigationPage
{
    private SearchBarViewModel _viewModel;
    public SearchBarMaterial3ControlPage()
    {
...
public partial class SearchBarMaterial3ControlMainPage : ContentPage
{
    ...
    public SearchBarMaterial3ControlMainPage(SearchBarViewModel viewModel)
    {
        ...
        DisableCursorVisibility(searchBar);
    }

    /// <summary>
    /// Disables cursor blinking on the SearchBar to prevent flaky screenshot comparisons.
    /// Uses HandlerChanged to access the platform view after the handler is connected.
    /// </summary>
    static void DisableCursorVisibility(SearchBar searchBar)
    {
        searchBar.HandlerChanged += (s, e) =>
        {
#if ANDROID
            if (searchBar.Handler?.PlatformView is Google.Android.Material.TextField.TextInputLayout layout)
            {
                // Material3 SearchBar (SearchBarHandler2) uses TextInputLayout with EditText
                layout.EditText?.SetCursorVisible(false);
            }
#elif IOS || MACCATALYST
            if (searchBar.Handler is Microsoft.Maui.Handlers.SearchBarHandler handler)
            {
